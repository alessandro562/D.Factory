...
() => {
  ...
  for (const slide of document.querySelectorAll('.slide')) {
    ...
    while ((n = walk.nextNode())) {
      ...
      if (svg) {
        ...
        if (vb && vb.width && vb.height) {
          // preserveAspectRatio=meet: la scala e' la minore delle due,
          // non quella orizzontale. Con width 1280 su viewBox 1280 ma
          // height 440 su 480 il testo rimpicciolisce del 9%.
    ...
    // ---- contenuto tagliato dal viewBox di un SVG ----
    // Il clipping dentro un <svg> non e' un overflow del DOM: il browser lo
    // nasconde e basta. Va misurato con getBBox contro il viewBox.
    for (const svg of slide.querySelectorAll('svg')) {
      const vb = svg.viewBox && svg.viewBox.baseVal;
      if (!vb || !vb.width) continue;
      let bb;
      try { bb = svg.getBBox(); } catch (e) { continue; }
      const dx = Math.round((bb.x + bb.width) - (vb.x + vb.width));
      const dy = Math.round((bb.y + bb.height) - (vb.y + vb.height));
      if (dx > 1 || dy > 1) {
        let txt = '';
        for (const t of svg.querySelectorAll('text')) {
          const r = t.getBBox();
          if (r.y + r.height > vb.y + vb.height + 1) { txt = t.textContent.trim(); break; }
        }
        rec.clipped.push({ id: svg.getAttribute('aria-labelledby') || '?',
                           dx: Math.max(0, dx), dy: Math.max(0, dy), txt: txt.slice(0, 44) });
      }
    }
...
# ---------------------------------------------------------------------------
# V9.3 · il §19 ordina di rimuovere la regola del 55-70% su ogni canvas.
# Al suo posto: un intervallo per tipologia, dichiarato sul markup con
# data-tipo, e un tetto di parole per tipologia (§11.2 e §2.2 del master).
# (parole max, visuale min, visuale max, min copy px)
# ---------------------------------------------------------------------------
# La colonna «visuale minima» non esiste piu': il §19 ordina di rimuovere la
...
def report(html, default='sales'):
    ...
    for r in data:
        ...
        if r['visualPct'] > vmax: flags.append(f'visual>{vmax}%')
        if tipo == 'pacchetto' and r.get('uiViews'): flags.append('DASHBOARD-IN-PACCHETTO')
        if flags: bad += 1
        tot += copy
        ...
        for c in r.get('clipped', [])[:3]:
            print(f'      ! contenuto tagliato dal viewBox {c["id"]} dx={c["dx"]} dy={c["dy"]} "{c["txt"]}"')
